...
const WeatherWidget = () => {
  ...
  const getBgColor = (main) => {
    switch ((main || "").toLowerCase()) {
      case "clear":
        return "from-yellow-100 via-yellow-50 to-white";
      case "clouds":
      ...
      case "thunderstorm":
        return "from-indigo-200 via-indigo-100 to-white";
      default:
  ...
  if (error)
    return (
      <div className="absolute top-5 right-7 z-20 bg-white/70 backdrop-blur-md rounded-2xl shadow-lg px-7 py-4 text-red-500 text-base border border-red-200">
        {error}
      </div>
    );
  ...
  return (
    <div
      className={`absolute top-5 right-7 z-20 bg-gradient-to-br ${bgColor} bg-opacity-80 backdrop-blur-md rounded-2xl shadow-xl px-7 py-4 flex items-center gap-6 min-w-[230px] border border-blue-200 hover:shadow-2xl hover:brightness-105 transition-all duration-300 group`}
      style={{ boxShadow: "0 4px 24px 0 rgba(59,130,246,0.10)" }}
    >
      {/* Soft background circle */}
      <div
        className="absolute left-2 top-2 w-14 h-14 rounded-full opacity-20 -z-0"
        style={{
          background:
            "radial-gradient(circle at 60% 40%, #38bdf8 0%, #a5b4fc 80%, transparent 100%)",
        }}
      ></div>
      <img
      ...
      <div className="relative z-10 flex flex-col">
        <span className="text-2xl font-extrabold text-blue-700 leading-6">
          {Math.round(weather.main.temp)}°C
        </span>
        <span className="text-base text-gray-700 font-bold leading-5">
          {weather.name}
        </span>
        <span className="text-sm text-blue-500/80 font-medium capitalize leading-4">
    ...
    const fetchAccountInfo = async () => {
      try {
        ...
        if (response && response.success) {
          let accountList = Array.isArray(response.data)
            ? response.data
            : [response.data];
          // TRY hesabı varsa ilk sıraya al
          const tryIndex = accountList.findIndex(
