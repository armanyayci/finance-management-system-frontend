...
const CurrencyList = () => {
  ...
  const currencyKeys = Object.keys(currencies).filter(
    (key) => key !== "" && currencies[key]?.Alış
  );
  ...
  currencyKeys.forEach((key) => {
    const change = parseFloat(
      (currencies[key].Değişim || "0").replace("%", "")
    );
    if (maxChange === null || change > maxChange.value)
      maxChange = { key, value: change };
    if (minChange === null || change < minChange.value)
      minChange = { key, value: change };
  });
    ...
    currencyKeys.forEach((key) => {
      if (prevCurrencies[key] && currencies[key]) {
        if (currencies[key].Alış !== prevCurrencies[key].Alış) {
          newHighlighted[key] =
            currencies[key].Alış > prevCurrencies[key].Alış ? "up" : "down";
        }
      }
    });
  ...
  useEffect(() => {
    const fetchUserAccount = async () => {
      try {
        ...
        if (username) {
          const accountData = await ApiService.GetAccountInfoByUsername(
            username
          );
    ...
    const fetchUserIdAndCurrencies = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          if (
            userCurrenciesRes.success &&
            Array.isArray(userCurrenciesRes.data)
          ) {
    ...
    if (tab === 1) {
      const fetchUserIdAndCurrencies = async () => {
        ...
        try {
          ...
          if (user) {
            setUserId(user.id);
            const userCurrenciesRes = await ApiService.getUserCurrencies(
              user.id
            );
            if (
              userCurrenciesRes.success &&
  ...
  const parseChange = (str) =>
    parseFloat((str || "0").replace("%", "").replace(",", "."));
  ...
  const handleConfirm = async () => {
    try {
      if (transactionType === "Buy") {
        ...
        const response = await ApiService.buyCurrency(
        ...
        if (username) {
          const updatedAccountData = await ApiService.GetAccountInfoByUsername(
            username
          );
        ...
        const response = await ApiService.sellCurrency(
          currencyConversionRequest
        );
        if (response.success) {
          ...
          if (username) {
            const updatedAccountData =
              await ApiService.GetAccountInfoByUsername(username);
            setUserAccount(updatedAccountData);
          }
          window.location.reload();
        } else {
  ...
  const fixPrice = (priceStr) =>
    priceStr ? priceStr.replace(/\./g, "").replace(",", ".") : "0";
  ...
  return (
    <Box
      sx={{
        minHeight: "100vh",
        background:
          "linear-gradient(135deg, #232946 0%, #3b82f6 60%, #f0fdfa 100%)",
        py: isMobile ? 1 : 2,
        px: isMobile ? 0 : 1,
        position: "relative",
        overflow: "hidden",
    ...
    >
      ...
      <Box
        sx={{
          ...
          opacity: 0.25,
          background:
            "radial-gradient(circle at 60% 40%, #6366f1 0%, #0ea5e9 60%, transparent 100%)",
        }}
      />
      <Paper
        elevation={12}
      ...
      >
        ...
        >
          <Tooltip title="View your owned currencies and balances" arrow>
            ...
            >
              <AccountBalanceIcon
                sx={{ fontSize: 28, color: "white", mr: 1 }}
              />
              <span
                style={{
                  fontWeight: 800,
          ...
          >
            <Typography
              sx={{ fontWeight: 800, fontSize: 18, mb: 1, color: "#0e7490" }}
            >
              My Currencies
            </Typography>
        ...
        >
          ...
          >
            <ArrowBackIosNewIcon
              sx={{ color: "#e0e7ff", fontWeight: "bold", fontSize: 32 }}
            />
          </Button>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            ...
            <Typography
              ...
              fontWeight={900}
              color="#e0e7ff"
              sx={{ letterSpacing: 2, fontSize: isMobile ? 26 : 32 }}
            >
              Currency Exchange
            </Typography>
          </Box>
        ...
        >
          <Box sx={{ display: "flex", gap: 2 }}>
            ...
            {maxChange && (
              <Chip
                label={`Top Gainer: ${maxChange.key} (${
                  currencies[maxChange.key]?.Değişim || ""
                })`}
                color="success"
                sx={{
                  fontWeight: 800,
            ...
            {minChange && (
              <Chip
                label={`Top Loser: ${minChange.key} (${
                  currencies[minChange.key]?.Değişim || ""
                })`}
                color="error"
                sx={{
                  fontWeight: 800,
          ...
          <TextField
            ...
            sx={{
              ...
              borderRadius: 2,
              color: "#e0e7ff",
              input: { color: "#e0e7ff", fontWeight: 700, fontSize: 17 },
            }}
            InputProps={{
              style: { color: "#e0e7ff", fontWeight: 700, fontSize: 17 },
            }}
          />
        </Box>
        {/* Table */}
          ...
          <Table>
            ...
            <TableBody>
              {sortedKeys
                ...
                .map((key) => {
                  const isAlisUp =
                    parseFloat(currencies[key].Alış) >
                    (parseFloat(prevCurrencies[key]?.Alış) || 0);
                  const isSatisUp =
                    parseFloat(currencies[key].Satış) >
                    (parseFloat(prevCurrencies[key]?.Satış) || 0);
                      ...
                      <TableCell>
                        <Chip
                          ...
                          sx={{
                            ...
                            boxShadow: highlight
                              ? highlight === "up"
                                ? "0 0 12px 2px #22c55e55"
                                : "0 0 12px 2px #ef444455"
                              : undefined,
                          }}
                          icon={
                            isChangeDown ? (
                              <TrendingDown
                                sx={{ color: "#b91c1c", fontSize: 22 }}
                              />
                            ) : (
                              <TrendingUp
                                sx={{ color: "#166534", fontSize: 22 }}
                              />
                            )
                          }
                        />
        ...
        >
          ...
          <DialogContent sx={{ pt: 1 }}>
            ...
            >
              Current {transactionType} Price:{" "}
              <span style={{ color: "#0ea5e9", fontWeight: 700 }}>
                {transactionType === "Buy"
                  ? currencies[selectedCurrency]?.Alış
                  : currencies[selectedCurrency]?.Satış}
              </span>
            </Typography>
            ...
            {transactionType === "Sell" && (
              <Chip
                label={`Available: ${
                  userCurrencies.find(
                    (c) => c.currencyName === selectedCurrency
                  )?.amount || 0
                }`}
                color="info"
                sx={{
                ...
                <span style={{ color: "#16a34a", fontWeight: 700 }}>
                  {(
                    ...
                    (transactionType === "Buy"
                      ...
                      : parseFloat(
                          fixPrice(currencies[selectedCurrency]?.Satış)
                        ))
                  ).toFixed(4)}{" "}
                  ₺
                </span>
